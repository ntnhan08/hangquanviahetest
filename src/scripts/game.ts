// ============================================================
// game.ts — Lõi game Hàng Quán Vỉa Hè (NÂNG CẤP)
// Vòng lặp, thế giới, mini-game, phục vụ, hạt, ambient & toàn bộ UI.
// Nâng cấp: Auto-focus UI, đồ họa chi tiết pixel-perfect,
// công thức phức tạp, customer flow thông minh, hiệu ứng & animation
// ============================================================
...
// bảng màu "quán Việt cổ × hiện đại" - nâng cấp gradient thơ mộng
const C = {
  paper: "#f3e3c0",
  paperDk: "#e6d0a4",
  ink: "#33220f",
...
export class Game {
  ...
  private missed = 0;
  private spawnT = 2;
  private lastCustomerLeaveTime = 0; // Thời điểm khách cuối cùng rời khỏi cửa
  private spawnBlockedUntil = 0; // Chặn spawn cho đến thời điểm này
  ...
  private updateCustomers(dt: number, full: boolean): void {
    for (let i = this.customers.length - 1; i >= 0; i--) {
      ...
      if (res === "expired") {
        ...
        // Khách đã rời khỏi cửa → đánh dấu thời điểm và mở lại spawn sau 10s
        this.lastCustomerLeaveTime = this.clock;
        this.spawnBlockedUntil = this.clock + 10; // 10 giây sau mới có khách mới
  ...
  private update(dt: number): void {
    ...
    this.spawnT -= dt;
    if (this.spawnT <= 0) {
      // Chỉ spawn khi: không bị chặn và còn bàn trống
      if (this.clock >= this.spawnBlockedUntil) {
        const freeTables = this.cfg.tables - this.customers.filter(c => c.state !== "leave").length;
        if (freeTables > 0) {
          this.spawnT = rand(this.cfg.spawnMin, this.cfg.spawnMax);
          this.trySpawn();
        } else {
          // Full bàn → ngưng spawn cho đến khi có bàn trống
          this.spawnT = 0.5; // kiểm tra lại sau 0.5s
        }
      } else {
        // Vẫn trong thời gian chặn spawn (10s sau khi khách cuối rời đi)
        this.spawnT = Math.min(this.spawnT + dt, this.cfg.spawnMax);
      }
    }
